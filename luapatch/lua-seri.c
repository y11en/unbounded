...
// 自己补环境
static inline int 
lua_rawgeti_(lua_State* L, int n, int id)
{
	lua_rawgeti(L, n, id);
	return lua_type(L, -1);
}
...
lookup_ref(lua_State *L, struct write_block *b, const void *obj) {
	if (b->s.objectid <= MAX_REFERENCE) {
		...
		lua_pop(L, 1);
		if (lua_rawgeti_(L, b->s.ref_index + 1, id) == LUA_TLIGHTUSERDATA) {
...
unpack_ref(lua_State *L, struct read_block *rb, int ref) {
	...
	if (ref == EXTEND_NUMBER) {
		...
		if (lua_type(L, s->ref_index) != LUA_TTABLE || lua_rawgeti_(L, s->ref_index, id) != LUA_TTABLE) {
